# 信息图结构组件生成规范（my-blog 版）

> **my-blog 项目说明**：本 reference 来自 cosZone/astro-koharu 项目移植。在 my-blog 中：
> - Structure 组件落点：`d:/bot/my-blog/src/designs/structures/[Name].tsx`（CamelCase）
> - 注册名：小写连字符 + 分类前缀（如 `list-row`、`hierarchy-tree`）
> - 用户在博文里通过 YAML body 的 `template` 字段指向自定义 structure 注册名
> - 输出 YAML body（不是独立 HTML 文件），玻璃 theme 自动注入（无需手写 themeConfig）
