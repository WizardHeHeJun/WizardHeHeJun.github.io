# 信息图数据项组件生成规范（my-blog 版）

> **my-blog 项目说明**：本 reference 来自 cosZone/astro-koharu 项目移植。在 my-blog 中：
> - Item 组件落点：`d:/bot/my-blog/src/designs/items/[Name].tsx`（CamelCase）
> - 注册名：小写连字符（如 `done-list`）
> - 用户在博文里通过 YAML body 的 `template` + `data.items[i].itemType`（如模板支持）触发自定义 item
